...
def get_rent(db: Session, id: int):
...
def get_rents_by_userid(db: Session, userid: int):
    return db.query(Rent).filter(Rent.userid == userid).all()

def get_rents_by_orgid(db: Session, orgid: int):
...
def create_rent(db: Session, rent: RentCreate):
